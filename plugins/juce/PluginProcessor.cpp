...
juce::AudioProcessorValueTreeState::ParameterLayout RaptorBreakcoreAudioProcessor::createParameterLayout() {
    ...
    layout.add(std::make_unique<juce::AudioParameterChoice>("oversamplingFactor", "Oversampling", juce::StringArray{"1x", "2x", "4x"}, 0));
    addFloat(layout, "postLowPassHz", "Anti-Alias LPF", 6000.0F, 22000.0F, 16000.0F);
...
void RaptorBreakcoreAudioProcessor::syncParametersFromAPVTS() {
    ...
    const int oversampleChoice = static_cast<int>(readParam(apvts_, "oversamplingFactor", 0.0F));
    advanced.oversamplingFactor = (oversampleChoice == 1) ? 2 : ((oversampleChoice == 2) ? 4 : 1);
    advanced.postLowPassHz = readParam(apvts_, "postLowPassHz", 16000.0F);
